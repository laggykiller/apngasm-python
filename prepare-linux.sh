...
mkdir build
cd ./build
cmake -DCMAKE_C_FLAGS="${CMAKE_C_FLAGS} -O3 -fPIC" -DCMAKE_INSTALL_PREFIX:PATH=/tmp/zlib ..
make install -j
...
mkdir build
cd ./build
cmake -DCMAKE_C_FLAGS="${CMAKE_C_FLAGS} -O3 -fPIC" -DCMAKE_POLICY_DEFAULT_CMP0074=NEW -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX:PATH=/tmp/libpng -DPNG_SHARED=OFF -DZLIB_ROOT=/tmp/zlib -DZLIB_USE_STATIC_LIBS=ON ..
make install -j
