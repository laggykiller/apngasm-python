...
class APNGAsmBinder:
    ...
    def frame_pixels_as_pillow(
        ...
        :param Optional[PIL.Image.Image] new_value: If set, then
            the raw pixel data of frame
    ...
    def frame_pixels_as_numpy(
        ...
        :param int frame: Target frame number.
        :param Optional[numpy.typing.NDArray] new_value: If set, then
            the raw pixel data of frame
            is set with this value.

        :return: 3D numpy array representation of raw pixel data of frame (get)
            or None (set)
    ...
    def frame_width(self, frame: int, new_value: Optional[int] = None) -> Optional[int]:
        ...
        :param int frame: Target frame number.
        :param Optional[int] new_value: If set, then
            the width of frame
            is set with this value.
    ...
    def frame_palette(
        ...
        :param int frame: Target frame number.
        :param Optional[numpy.typing.NDArray] new_value: If set, then
            the palette data of frame
            is set with this value.

        :return: 2D numpy array representation of palette data of frame (get)
            or None (set)
        :rtype: Optional[numpy.typing.NDArray]
        """
        if new_value:
    ...
    def frame_transparency(
        ...
        :param int frame: Target frame number.
        :param Optional[numpy.typing.NDArray] new_value: If set, then
            the transparency of frame is set with this value.

        :return: The color [r, g, b] to be treated as transparent in the frame (get)
            or None (set)
        :rtype: Optional[numpy.typing.NDArray]
        """
        if new_value:
    ...
    def frame_transparency_size(
        ...
        :return: Transparency data size of frame (get)
            or None (set)
        :rtype: Optional[int]
        """
        if new_value:
    ...
    def add_frame_from_numpy(
        ...
        :param numpy.typing.NDArray numpy_data: The pixel data,
        ...
        :param Optional[str] mode: The color mode of data.
            Possible values are RGB or RGBA.
            If not given, it is determined using
            the 3rd dimension size of numpy_data.
        :param Optional[numpy.typing.NDArray] trns_color: The color [r, g, b]
        ...
        if not mode:
            if np.shape(numpy_data)[2] == 3:
                ...
                raise TypeError(
                    ...
                    f"The 3rd dimension size was {np.shape(numpy_data)[2]}."
                )
