...
mkdir build
cd build
cmake -G "Visual Studio 17 2022" -DCMAKE_C_FLAGS_RELEASE="/MT" -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX:PATH=C:\opt\zlib ..
...
mkdir build
cd build
cmake -G "Visual Studio 17 2022" -DCMAKE_C_FLAGS_RELEASE="/MT" -DBUILD_SHARED_LIBS=OFF -DCMAKE_POLICY_DEFAULT_CMP0074=NEW -DCMAKE_INSTALL_PREFIX:PATH=C:\opt\libpng -DZLIB_ROOT=C:\opt\zlib -DZLIB_USE_STATIC_LIBS=ON -DPNG_SHARED=OFF ..
cmake --build . --target INSTALL --config Release
